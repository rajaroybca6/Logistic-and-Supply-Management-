...
import numpy as np
import os
import urllib.request  # Added for cloud model download
...
# --- MODEL LOADING (CLOUD-OPTIMIZED HANDSHAKE) ---
@st.cache_resource
def load_model():
    # Look for the file in the root folder of the Streamlit server
    model_file = "logistic_guardian_v3_2.pkl"

    # Direct link to the asset in your live GitHub Release v1.0.0
    url = "https://github.com/rajaroybca6/Logistic-and-Supply-Management-/releases/download/v1.0.0/logistic_guardian_v3_2.pkl"

    # Download if missing on the Streamlit server (Fixes path error)
    if not os.path.exists(model_file):
        try:
            with st.spinner("📥 Downloading 56MB model from GitHub Release..."):
                urllib.request.urlretrieve(url, model_file)
        except Exception as e:
            st.error(f"❌ Download Failed. Check your Release Tag name: {e}")
            return None
...
if pipeline is None:
    st.error("❌ Model file not found. Please ensure your GitHub Release v1.0.0 contains 'logistic_guardian_v3_2.pkl'.")
else:
    ...
    with tab6:
        ...
        if active_df is None:
            ...
            if c1.button("✅ Add GPS"): sim.loc[0, "tracking_gps"] = 1
            if c2.button("❌ GPS Off"): sim.loc[0, "tracking_gps"] = 0
            if c3.button("➖ Transit -1"): sim.loc[0, "numero_transiti"] = max(0, int(sim.loc[0, "numero_transiti"]) - 1)
            if c4.button("➕ Transit +1"): sim.loc[0, "numero_transiti"] += 1
            if c5.button("📦 Fragile ON"): sim.loc[0, "fragile"] = 1
            if c6.button("📦 Fragile OFF"): sim.loc[0, "fragile"] = 0
            ...
            for role, content in st.session_state.chat_history: st.chat_message(role).markdown(content)
            if user_msg := st.chat_input("Ask about delay reasons, what-if scenarios, mitigation..."):
                ...
                if "weather" in msg_lower: reply = "Checking weather patterns... Switching to Rail might help."
